...
namespace MeshBrowser
{
    ...
    public partial class MeshBrowserForm : Form
    {
        ...
        private void InitMatrices()
        {
            ...
            projectionMatrix = dxUtil.Camera.MatrixPerspectiveFovLH((float)Math.PI * 0.25f, ((float)directControl.ClientSize.Width / (float)directControl.ClientSize.Height), 0.5f, 1000.0f);
        ...
        protected void RenderScene()
        {
            ...
            swapChain.TryPresent(1, PresentFlag.Default, out error);
        }
        #endregion
        ...
        {
            XDocument nowKnown = new XDocument(
                new XElement("KnownFiles",
                new XElement("Valid"),
                new XElement("Invalid")));
            var q1 = from files in knownFiles.Root.XPathSelectElements("./Invalid/File")
                     where (string)files.Attribute("path") == filename
                     select files;
            q1.Remove();
            var q2 = from files in knownFiles.Root.XPathSelectElements("./Valid/File")
                     where (string)files.Attribute("path") == filename
                     select files;
            q2.Remove();
