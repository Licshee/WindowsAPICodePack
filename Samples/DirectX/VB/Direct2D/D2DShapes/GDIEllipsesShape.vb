...
Namespace D2DShapes
	Friend Class GDIEllipsesShape
		...
		Private Shared Function RenderTargetSupportsGDI(ByVal rt As RenderTarget) As Boolean
			Dim propertiesToSupport = New RenderTargetProperties(RenderTargetType.Default, New PixelFormat(Format.B8G8R8A8_UNORM, AlphaMode.Ignore), 96, 96, RenderTargetUsage.GdiCompatible, FeatureLevel.Default)
			If rt.IsSupported(propertiesToSupport) Then
				Return True
			End If
			propertiesToSupport = New RenderTargetProperties(RenderTargetType.Default, New PixelFormat(Format.B8G8R8A8_UNORM, AlphaMode.Premultiplied), 96, 96, RenderTargetUsage.GdiCompatible, FeatureLevel.Default)
