...
Namespace TextureSwap
	...
	Partial Public Class TexturizerDemo
		...
		Private Sub InitDevice()
			...
			device = D3DDevice.CreateDeviceAndSwapChain(renderHost.Handle, swapChain)
			...
			Dim descDepth As New Texture2DDescription() With {.Width = CUInt(renderHost.ActualWidth), .Height = CUInt(renderHost.ActualHeight), .MipLevels = 1, .ArraySize = 1, .Format = Format.D32_FLOAT, .SampleDescription = New SampleDescription() With {.Count = 1, .Quality = 0}, .BindFlags = BindFlag.DepthStencil}
		...
		Protected Sub RenderScene()
			...
			' present back buffer
			swapChain.Present(1, PresentFlag.Default)
		End Sub
		#End Region
		...
		Private Sub Window1_SizeChanged(ByVal sender As Object, ByVal e As SizeChangedEventArgs)
			If device IsNot Nothing Then
				...
				swapChain.ResizeBuffers(sd.BufferCount, CUInt(renderHost.ActualWidth), CUInt(renderHost.ActualHeight), sd.BufferDescription.Format, sd.Flags)
