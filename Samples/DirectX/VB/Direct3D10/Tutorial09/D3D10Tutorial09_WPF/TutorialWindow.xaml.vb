...
Namespace D3D10Tutorial09_WPF
	...
	Partial Public Class TutorialWindow
		...
		Public Sub InitDevice()
			device = D3DDevice.CreateDeviceAndSwapChain(host.Handle, swapChain)
        ...
        Private Sub SetViews()
            ...
            Dim descDepth As New Texture2DDescription() With {.Width = CUInt(host.ActualWidth), .Height = CUInt(host.ActualHeight), .MipLevels = 1, .ArraySize = 1, .Format = Format.D32_FLOAT, .SampleDescription = New SampleDescription() With {.Count = 1, .Quality = 0}, .BindFlags = BindFlag.DepthStencil}
