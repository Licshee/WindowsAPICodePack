...
Namespace D3D10Tutorial02_WinFormsControl
	...
	Partial Public Class TutorialWindow
		...
		Protected Sub InitDevice()
			...
			Dim layout() As InputElementDescription = { New InputElementDescription() With {.SemanticName = "POSITION", .SemanticIndex = 0, .Format = Format.R32G32B32_FLOAT, .InputSlot = 0, .AlignedByteOffset = 0, .InputSlotClass = InputClassification.PerVertexData, .InstanceDataStepRate = 0} }
			...
			Dim bd As New BufferDescription() With {.Usage = Usage.Default, .ByteWidth = CUInt(Marshal.SizeOf(vertex)), .BindFlags = BindFlag.VertexBuffer, .CpuAccessFlags = 0, .MiscFlags = 0}

			Dim vertexData As IntPtr = Marshal.AllocCoTaskMem(Marshal.SizeOf(vertex))
			Marshal.StructureToPtr(vertex, vertexData, False)

			Dim InitData As New SubresourceData() With {.SysMem = vertexData, .SysMemPitch = 0, .SysMemSlicePitch = 0}
		...
		Private Sub SetViews()
			...
			device.OM.SetRenderTargets(New RenderTargetView() { renderTargetView }, Nothing)
