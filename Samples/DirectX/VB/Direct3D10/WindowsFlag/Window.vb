...
Namespace WindowsFlag
	...
	Partial Public Class Window
        ...
        Protected Sub InitializeDevice()
            device = D3DDevice.CreateDeviceAndSwapChain(directControl.Handle, swapChain)
...
#End Region
        ...
        Private Sub SetViews()
            ...
            Dim descDepth As New Texture2DDescription() With {.Width = CUInt(directControl.ClientSize.Width), .Height = CUInt(directControl.ClientSize.Height), .MipLevels = 1, .ArraySize = 1, .Format = Format.D32_FLOAT, .SampleDescription = New SampleDescription() With {.Count = 1, .Quality = 0}, .BindFlags = BindFlag.DepthStencil}
        ...
        Private Sub InitializeVertexBuffer()
            ...
            Dim bufferDesc As New BufferDescription() With {.Usage = Usage.Default, .ByteWidth = CUInt(Marshal.SizeOf(cube.Vertices)), .BindFlags = BindFlag.VertexBuffer, .CpuAccessFlags = 0, .MiscFlags = 0}

            Dim InitData As New SubresourceData() With {.SysMem = verticesData}
...
#End Region
        ...
        Private Sub InitializeIndexBuffer()
            ...
            Dim bufferDesc As New BufferDescription() With {.Usage = Usage.Default, .ByteWidth = CUInt(Marshal.SizeOf(cube.Indices)), .BindFlags = BindFlag.IndexBuffer, .CpuAccessFlags = 0, .MiscFlags = 0}

            Dim initData As New SubresourceData() With {.SysMem = indicesData}
        ...
        Protected Sub RenderScene()
            SyncLock viewSync
                ...
                If (needsResizing) Then
                    ...
                    swapChain.ResizeBuffers(sd.BufferCount, CType(directControl.ClientSize.Width, UInteger), CType(directControl.ClientSize.Height, UInteger), sd.BufferDescription.Format, sd.Flags)
                    SetViews()
                    effects.ProjectionMatrix = Microsoft.WindowsAPICodePack.DirectX.DirectXUtilities.Camera.MatrixPerspectiveFovLH(CSng(Math.PI) * 0.25F, (CSng(Me.ClientSize.Width) / CSng(Me.ClientSize.Height)), 0.1F, 4000.0F)
                End If
                ...
                device.ClearDepthStencilView(depthStencilView, ClearFlag.Depth, 1.0F, CByte(0))
