...
namespace Microsoft.WindowsAPICodePack.Samples
{
    public partial class ExplorerBrowserTestForm : Form
    {
        public ExplorerBrowserTestForm()
        {
            ...
            explorerBrowser.NavigationPending += new ExplorerBrowserNavigationPendingEvent( explorerBrowser_NavigationPending );
            explorerBrowser.NavigationFailed += new ExplorerBrowserNavigationFailedEvent( explorerBrowser_NavigationFailed );
            explorerBrowser.NavigationComplete += new ExplorerBrowserNavigationCompleteEvent( explorerBrowser_NavigationComplete );
            explorerBrowser.ItemsChanged += new ExplorerBrowserItemsChangedEvent( explorerBrowser_ItemsChanged );
            explorerBrowser.SelectionChanged += new ExplorerBrowserSelectionChangedEvent( explorerBrowser_SelectionChanged );

            // set up Navigation log event and button state
            explorerBrowser.NavigationLog.NavigationLogChanged += new NavigationLogChangedEvent( NavigationLog_NavigationLogChanged );
        ...
        void explorerBrowser_SelectionChanged( object sender, EventArgs e )
        {
            ...
            BeginInvoke( new MethodInvoker( delegate( )
            {
                ...
                {
                    if( item != null )
                        itemsText.AppendLine( "\tItem = " + item.GetDisplayName( DisplayNameType.Normal ) );
                }
        ...
        void explorerBrowser_NavigationComplete( object sender, NavigationCompleteArgs args )
        {
            // This event is BeginInvoked to decouple the ExplorerBrowser UI from this UI
            BeginInvoke( new MethodInvoker( delegate( )
            {
        ...
        void explorerBrowser_NavigationFailed( object sender, NavigationFailedArgs args )
        {
            // This event is BeginInvoked to decouple the ExplorerBrowser UI from this UI
            BeginInvoke( new MethodInvoker( delegate( )
            {
        ...
        void explorerBrowser_NavigationPending( object sender, NavigationPendingArgs args )
        {
        ...
        private void navigateButton_Click( object sender, EventArgs e )
        {
            try
            {
                ...
                explorerBrowser.Navigate( FileSystemFolder.FromFolderPath( pathEdit.Text ) );
            }
            catch( COMException )
            {
