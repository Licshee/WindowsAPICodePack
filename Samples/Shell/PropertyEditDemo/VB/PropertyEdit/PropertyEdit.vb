...
Namespace Microsoft.WindowsAPICodePack.Shell.Samples
    Public Class PropertyEdit
        ...
        Private Shared Sub DisplayPropertyValue(ByVal prop As IShellProperty)
            ...
            value = If(prop.ValueAsObject Is Nothing, "", prop.FormatForDisplay(PropertyDescriptionFormat.Default))
        ...
        Private Shared Sub ShowPropertyInfo(ByVal propertyName As String, ByVal propDesc As ShellPropertyDescription)
            ...
            Console.WriteLine(Constants.vbTab & "Has Multiple Values:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.MultipleValues) = PropertyTypeFlags.MultipleValues)
            Console.WriteLine(Constants.vbTab & "Is Group:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.IsGroup) = PropertyTypeFlags.IsGroup)
            Console.WriteLine(Constants.vbTab & "Is Innate:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.IsInnate) = PropertyTypeFlags.IsInnate)
            Console.WriteLine(Constants.vbTab & "Is Queryable:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.IsQueryable) = PropertyTypeFlags.IsQueryable)
            Console.WriteLine(Constants.vbTab & "Is Viewable:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.IsViewable) = PropertyTypeFlags.IsViewable)
            Console.WriteLine(Constants.vbTab & "Is SystemProperty:  {0}", (propDesc.TypeFlags And PropertyTypeFlags.IsSystemProperty) = PropertyTypeFlags.IsSystemProperty)
        End Sub
