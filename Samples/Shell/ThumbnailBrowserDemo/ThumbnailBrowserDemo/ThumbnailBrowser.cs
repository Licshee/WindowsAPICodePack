...
namespace Microsoft.WindowsAPICodePack.Samples.ThumbnailBrowserDemo
{
    public partial class ThumbnailBrowser : Form
    {
        ...
        public ThumbnailBrowser()
        {
            ...
            //
            explorerBrowser1.SelectionChanged += new ExplorerBrowserSelectionChangedEvent(explorerBrowser1_SelectionChanged);
            ...
            td = new TaskDialog();
            td.Caption = "Error displaying thumbnail";
            td.Instruction = "The selected item does not have a thumbnail and you have selected the viewing mode to be thumbnail only. Please select one of the following options:";
            td.StartupLocation = TaskDialogStartupLocation.CenterOwner;
            td.MainIcon = TaskDialogStandardIcon.Error;
            td.Cancelable = true;
            td.CheckBoxText = "Do not show this dialog again";
            td.CheckBoxChecked = false;
                ...
                catch (InvalidOperationException)
                {
                    if (currentMode == Mode.ThumbnailOnly)
                    {
                        ...
                        if (showErrorTaskDialog)
                        {
                            ...
                            if (tdr.CustomButtonClicked == "noChangeButton")
                                onErrorChangeMode = false;
                            else
                                onErrorChangeMode = true;

                            showErrorTaskDialog = !tdr.CheckBoxChecked;
                        }
