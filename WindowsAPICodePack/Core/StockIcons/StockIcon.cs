...
namespace Microsoft.WindowsAPICodePack
{
    ...
    public class StockIcon : IDisposable
    {
        ...
        /// </summary>
        /// <value>A <see cref="Microsoft.WindowsAPICodePack.StockIconSizes"/> value.</value>
        ...
        private IntPtr GetHIcon()
        {
            ...
            CoreNativeMethods.StockIconOptions flags = CoreNativeMethods.StockIconOptions.Handle;
            
            // Based on the current settings, update the flags
            if (CurrentSize == StockIconSizes.Small)
                flags |= CoreNativeMethods.StockIconOptions.Small;
            else if (CurrentSize == StockIconSizes.ShellSize)
                flags |= CoreNativeMethods.StockIconOptions.ShellSize;
            else
                flags |= CoreNativeMethods.StockIconOptions.Large;  // default

            if (Selected)
                flags |= CoreNativeMethods.StockIconOptions.Selected;

            if (LinkOverlay)
                flags |= CoreNativeMethods.StockIconOptions.LinkOverlay;

            // Create a StockIconInfo structure to pass to the native method.
            CoreNativeMethods.StockIconInfo info = new CoreNativeMethods.StockIconInfo();
            info.StuctureSize = (UInt32)Marshal.SizeOf(typeof(CoreNativeMethods.StockIconInfo));

            // Pass the struct to the native method
            HRESULT hr = CoreNativeMethods.SHGetStockIconInfo(identifier, flags, ref info);
