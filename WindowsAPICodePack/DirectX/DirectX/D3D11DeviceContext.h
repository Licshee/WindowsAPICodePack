...
ref class VertexShaderPipelineStage;
    ...
    public: 
        ...
        /// <param name="DstX">The x offset between the source box location and the destination location.</param>
        /// <param name="DstY">The y offset between the source box location and the destination location. For a 1D subresource, this must be zero.</param>
        /// <param name="DstZ">The z offset between the source box location and the destination location. For a 1D or 2D subresource, this must be zero.</param>
        /// <param name="sourceResource">The source resource (see <see cref="D3DResource"/>)<seealso cref="D3DResource"/>.</param>
        /// <param name="sourceSubresource">Source subresource index.</param>
        /// <param name="SrcBox">A 3D box (see <see cref="Box"/>)<seealso cref="Box"/> that defines the source subresources that can be copied. If NULL, the entire source subresource is copied. The box must fit within the source resource.</param>
        void CopySubresourceRegion(D3DResource^ destinationResource, UInt32 destinationSubresource, UInt32 DstX, UInt32 DstY, UInt32 DstZ, D3DResource^ sourceResource, UInt32 sourceSubresource, Box^ SrcBox);
        ...
        void FinishCommandList(Boolean restoreDeferredContextState, [Runtime::InteropServices::Out] CommandList^ %outCommandList);
        ...
        void GetPredication([Runtime::InteropServices::Out] D3DPredicate^ %outPredicate, [Runtime::InteropServices::Out] Boolean %outPredicateValue);
        ...
        void Map(D3DResource^ resource, UInt32 subresource, MapOption mapType, MapFlag mapFlags, MappedSubresource^ mappedResource);
        ...
        void UpdateSubresource(D3DResource^ destinationResource, UInt32 destinationSubresource, Box^ DstBox, IntPtr SrcData, UInt32 SrcRowPitch, UInt32 SrcDepthPitch);
    ...
    internal:
        ...
        {
            nativeUnknown.Set(pNativeID3D11DeviceContext);
        }

        property ID3D11DeviceContext* NativeID3D11DeviceContext
        {
            ID3D11DeviceContext* get()
            {
                return reinterpret_cast<ID3D11DeviceContext*>(nativeUnknown.Get());
            }
        }
    private:
