...
public:
    ...
    D3DBuffer^ GetIndexBuffer( [Runtime::InteropServices::Out] Format %outFormat, [Runtime::InteropServices::Out] UInt32 %outOffset);
    ...
    /// <param name="outStrides">A collection of stride values returned by the method; one stride value for each buffer in the vertex-buffer array. Each stride value is the size (in bytes) of the elements that are to be used from that vertex buffer.</param>
    /// <param name="outOffsets">A collection of offset values returned by the method; one offset value for each buffer in the vertex-buffer array. Each offset is the number of bytes between the first element of a vertex buffer and the first element that will be used.</param>
    /// <returns>A collection of vertex buffers returned by the method (see <see cref="D3DBuffer"/>)<seealso cref="D3DBuffer"/>.</returns>
    Collection<D3DBuffer^>^ GetVertexBuffers(UInt32 startSlot, UInt32 numBuffers, [Runtime::InteropServices::Out] array<UInt32>^ %outStrides, [Runtime::InteropServices::Out] array<UInt32>^ %outOffsets);
