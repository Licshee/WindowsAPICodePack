...
Collection<D3DBuffer^>^ PixelShaderPipelineStage::GetConstantBuffers(UInt32 startSlot, UInt32 numBuffers)
{
    ...
    Parent->NativeID3D11DeviceContext->PSGetConstantBuffers(safe_cast<UINT>(startSlot), safe_cast<UINT>(numBuffers), &tempoutConstantBuffers[0]);
        
    return GetList<D3DBuffer, ID3D11Buffer>(numBuffers, tempoutConstantBuffers);
}

Collection<SamplerState^>^ PixelShaderPipelineStage::GetSamplers(UInt32 startSlot, UInt32 numSamplers)
{
    System::Diagnostics::Debug::Assert( Parent->NativeID3D11DeviceContext != NULL);

    vector<ID3D11SamplerState*> tempoutSamplers(numSamplers);
    Parent->NativeID3D11DeviceContext->PSGetSamplers(safe_cast<UINT>(startSlot), safe_cast<UINT>(numSamplers), &tempoutSamplers[0]);

    return GetList<SamplerState, ID3D11SamplerState>(numSamplers, tempoutSamplers);
}

PixelShader^ PixelShaderPipelineStage::GetShader(UInt32 numClassInstances, [Runtime::InteropServices::Out] Collection<ClassInstance^>^ %outClassInstances)
{
    System::Diagnostics::Debug::Assert(Parent->NativeID3D11DeviceContext != NULL);
    ...
    Parent->NativeID3D11DeviceContext->PSGetShader(
        &tempShader, 
...
Collection<ShaderResourceView^>^ PixelShaderPipelineStage::GetShaderResources(UInt32 startSlot, UInt32 numViews)
{
    System::Diagnostics::Debug::Assert( Parent->NativeID3D11DeviceContext != NULL);

    vector<ID3D11ShaderResourceView*> tempoutShaderResourceViews(numViews);
    Parent->NativeID3D11DeviceContext->PSGetShaderResources(safe_cast<UINT>(startSlot), safe_cast<UINT>(numViews), &tempoutShaderResourceViews[0]);
...
void PixelShaderPipelineStage::SetConstantBuffers(UInt32 startSlot, IEnumerable<D3DBuffer^>^ constantBuffers)
{
    ...
    Parent->NativeID3D11DeviceContext->PSSetConstantBuffers(
...
void PixelShaderPipelineStage::SetSamplers(UInt32 startSlot, IEnumerable<SamplerState^>^ samplers)
{
    ...
    Parent->NativeID3D11DeviceContext->PSSetSamplers(
        safe_cast<UINT>(startSlot), 
        count, 
        count == 0 ? NULL : &(itemsVector[0]));
}
...
void PixelShaderPipelineStage::SetShader(PixelShader^ shader, IEnumerable<ClassInstance^>^ classInstances)
{
    ...
    Parent->NativeID3D11DeviceContext->PSSetShader(
        shader == nullptr ? NULL : shader->NativeID3D11PixelShader, 
...
void PixelShaderPipelineStage::SetShaderResources(UInt32 startSlot, IEnumerable<ShaderResourceView^>^ shaderResourceViews)
{
    ...
    Parent->NativeID3D11DeviceContext->PSSetShaderResources(
        safe_cast<UINT>(startSlot), 
        count, 
        count == 0 ? NULL : &(itemsVector[0]));
}
