...
public:
    ...
    /// </returns>
    Collection<D3dRect^>^ GetScissorRects(UInt32 maxNumberOfRects);
    ...
    Collection<Viewport^>^ GetViewports(UInt32 maxNumberOfViewports);
    ...
    /// See the SDK for details about how the viewport size is dependent on the device feature level which has changed between Direct3D 11 and Direct3D 10.</param>
