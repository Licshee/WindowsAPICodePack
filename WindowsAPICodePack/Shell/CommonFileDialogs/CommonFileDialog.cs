...
namespace Microsoft.WindowsAPICodePack.Shell
{
    ...
    public abstract class CommonFileDialog : IDialogControlHost, IDisposable
    {
        ...
        /// <summary>
        /// Gets the selected item as ShellObject.
        /// </summary>
        /// <value>A ShellObject object.</value>
        ...
        public ShellObject Item
        {
            get
            {
                ...
                if (items.Count == 1)
                    return ShellObject.FromShellItem(items[0]);
                else
                    return null;
            }
        }
        ...
        /// places available for the user to open or save items. This methods actually adds an item
        /// to the "Favorite Links" or "Places" section of the Open/Save dialog.
        /// </summary>
        /// <param name="place">Item to add to the places list. Should of type IShellContainer</param>
        /// <param name="location">Location of where item needs to be placed in the list.</param>
        public void AddPlace(IShellContainer place, FileDialogAddPlaceLocation location)
        {
        ...
        /// </summary>
        /// <param name="path">Item to add to the places list.</param>
        /// <param name="location">Location of where item needs to be placed in the list.</param>
        ...
        /// dialog is shown. Null or an empty string will mean the dialog is using
        ...
        private IShellContainer initialLocation;
        /// <summary>
        /// Sets or gets a location (<see cref="Microsoft.WindowsAPICodePack.Shell.IShellContainer">IShellContainer</see>
        /// that is always selected when the dialog is opened, regardless of previous user action. 
        /// Null or an empty string will mean the dialog is using the default location.
        /// </summary>
        public IShellContainer InitialLocation
        {
            get
            {
        ...
        /// <summary>
        /// Sets the folder (as path) used as a default if there is not a recently used 
        ...
        private IShellContainer defaultLocation;
        /// <summary>
        /// Sets the location (<see cref="Microsoft.WindowsAPICodePack.Shell.IShellContainer">IShellContainer</see> 
        /// used as a default if there is not a recently used folder value available.
        /// </summary>
        public IShellContainer DefaultLocation
        {
        ...
        public CommonFileDialogResult ShowDialog()
        {
            ...
            if (CoreErrorHelper.Matches(hresult,
        ...
        private void ApplyNativeSettings(IFileDialog dialog)
        {
            ...
            if (parentWindow == null)
                parentWindow = CoreHelpers.GetDefaultOwnerWindow();
        ...
        void IDialogControlHost.ApplyControlPropertyChange(string propertyName, DialogControl control)
        {
            ...
            else if (propertyName == "Enabled" || propertyName == "Visible")
            {
                ...
                ShellNativeMethods.CDCONTROLSTATE state = ShellNativeMethods.CDCONTROLSTATE.CDCS_INACTIVE;
        ...
        internal static bool IsOptionSet(IFileDialog dialog, ShellNativeMethods.FOS flag)
        {
            ShellNativeMethods.FOS currentFlags = GetCurrentOptionFlags(dialog);

            return (currentFlags & flag) == flag;
        }

        internal static ShellNativeMethods.FOS GetCurrentOptionFlags(IFileDialog dialog)
        {
            ShellNativeMethods.FOS currentFlags;
        ...
        internal static string GetFileNameFromShellItem(IShellItem item)
        {
            ...
            item.GetDisplayName(ShellNativeMethods.SIGDN.SIGDN_DESKTOPABSOLUTEPARSING, out filename);
            return filename;
        }
        ...
        /// <summary>
        /// Called before <see cref="Microsoft.WindowsAPICodePack.Shell.CommonFileDialog.OnFolderChanged"/>. This allows the implementer to stop navigation to a particular location.
        /// </summary>
